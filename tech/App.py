...
class MyApp(MDApp):
    ...
    def on_start(self):
        ...
        # Liaison pour la vérification dynamique du mot de passe
    ...
    def get_prestations(self, type_prestation):
        data = {
            ...
            "en_cours": [
                {"titre": "Entretien espaces verts", "info": "Aujourd'hui", "type": "en_cours"},
                {"titre": "Maintenance serveur", "info": "Client Dupont", "type": "en_cours"},
            ],
            "a_venir": [
    ...
    def load_all_prestations(self):
        ...
        for container_id, p_type in [("container_prestations", "historique"), 
            ...
            for item in self.get_prestations(p_type):
                container.add_widget(self.create_prestation_item(item))

    def open_detail_screen(self, data):
        presta_type = data.get("type", "historique")
        
        if presta_type == "en_cours":
            target_screen, p = "detail_en_cours", "ec_" 
        elif presta_type == "a_venir":
            target_screen, p = "detail_a_venir", "av_"
        else:
            target_screen, p = "detail_historique", "hi_"
        ...
        # --- CHANGEMENT DEMANDÉ ICI ---
        if presta_type == "historique":
            ids.hi_title.text = f"Prestation du {data['info']}"
        else:
            ids[f"{p}title"].text = f"Prestation n°{self.current_prestation['numero']}"
        # ------------------------------

        ids[f"{p}subtitle"].text = self.current_prestation["titre"]
        ids[f"{p}client"].text = self.current_prestation["client"]
        ids[f"{p}technicien"].text = self.current_prestation["technicien"]
        ids[f"{p}adresse"].text = self.current_prestation["adresse"]
        ids[f"{p}infos"].text = self.current_prestation["infos"]
        
        if presta_type == "en_cours":
            ids.ec_date_debut.text = self.current_prestation["date_debut"]
            ids.ec_date_fin.text = self.current_prestation["date_fin"]
            ids.ec_complement.text = self.current_prestation["complement"]
    ...
    def submit_feedback(self):
        if self.root.ids.feedback_description.text:
    ...
    def tenter_connexion(self, email, mdp):
        if email == "remi" and mdp == "1234":
            ...
            self.root.ids.login_msg.text = "Identifiants incorrects."
